...
void monster_golem_bronze(void)
{
	...
	//Inky: 20200223 Reduce the golem size
	//setsize(self, '-60 -60 0', '60 60 190');
	setsize(self, '-32 -32 0', '32 32 96');
	GolemInit();
	if(!self.health)
		self.health = 650;
	if(!self.max_health)
...
void monster_golem_crystal(void)
{
	...
	//Inky 20200721 The next 3 precachings were missing in the vanilla game, that made the game crash if the golem was destroyed by an assassin's grenade
	precache_model3("models/goarm.mdl");
	precache_model3("models/golegs.mdl");
	precache_model3("models/g-head.mdl");
...
void GolemInit(void)
{
	...
	//Inky: 20200223 Speed was fixed to 2. Now customizable
	if(!self.speed)
		self.speed=2;
...
void GolemWalk(void) [++ $walk1..$walk34]
{
	...
	ai_walk(self.speed); //Inky: 20200223 Was ai_walk(2);
}
