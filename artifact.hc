...
void artifact_touch()
{
	...
	//Inky: 20200720 Legacy code put in the "else" case of this if block allowing to programmatically pick up the artifact
	if(self.use==artifact_touch)
	{
		other = activator;
		if(!other) return;
	}
	...
	//Inky: white servant
	else if(self.netname == STR_WHITE)
	{
		if ((other.cnt_haste + 1) > 15)
			return;	
		else
...
void spawn_artifact (float artifact,float respawnflag)
{
	...
	else if (artifact == ARTIFACT_WHITE) //Inky
		GenerateArtifactModel("models/a_servant.mdl",STR_WHITE,respawnflag);
...
//Inky
/*QUAKED art_white (.0 .0 .5) (-8 -8 -44) (8 8 20) FLOATING
Artifact for White servant
-------------------------FIELDS-------------------------
None
--------------------------------------------------------
*/
void art_white()
{
	spawn_artifact(ARTIFACT_WHITE,RESPAWN);
}
...
{
	entity dropped = DropBackpack();
	
	//Inky 20211213
	if(self.target) dropped.target = self.target;
	
	//Inky 20210823 Specific behavior for rewards
	if(self.spawnflags&1/*Reward*/)
	{
		dropped.origin = dropped.origin - '0 0 40';
		dropped.movetype = MOVETYPE_NONE;
		particleexplosion(dropped.origin,246/*color*/,3/*exploderadius*/,512/*counter*/);
		sound(dropped,CHAN_AUTO,"misc/reward.wav",1,self.speed);
	}
}
...
void item_spawner()
{
	if(self.spawnflags&1/*Reward*/) precache_sound ("misc/reward.wav");
	...
	if(self.spawnflags&2/*Once on startup*/)
		self.use();
}