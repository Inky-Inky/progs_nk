...
void player_sheep_snout_bite ()
{
	...
	//Inky: aggressive sheeps betray themselves!
...
void poly_touch ()
{
	setmodel ( self, "models/null.spr"); //Inky 20200619 Prevents zombie sprites staying forever
...
//Inky 20200619
void polymorph_anim_white_servant ()
{
	float particle_color;
	particle_color=406+random(8);
	particle4(self.origin,7,particle_color,PARTICLETYPE_EXPLODE2,random(1,5));
}

//Inky: replaced by the implementation by Brian Waak
/*
...
void FirePoly (float ofs,float ofsv)
{
	local vector vec = '0.00000 0.00000 0.00000';

	//OvinoImp mod code start
	//
	//Brought the newmis initialization forward, because the Imp's firing code doesn't use its viewing angle.
	//So I made code similar to the fireball code, which needs the missile to already exist. So the newmis
	//entity needs to be made first. This doesn't affect the PLAYER'S Polymorph code in any significant way.
	   ...
	   if ( self.classname != "monster_imp_lord" ) {
			//Assume this is a player using the Seal.
			makevectors ( self.v_angle);
			newmis.velocity = ((v_forward * newmis.speed) + (v_right * ofs));
			...
			//This is an Imp Lord firing sheep projectiles.
			...
			makevectors ( self.angles);
			vec = ((self.origin + self.view_ofs) + v_factor ( ('14.00000 8.00000 0.00000' * self.scale)));
			setorigin ( newmis, vec);
			vec = ((self.enemy.origin - newmis.origin) + self.enemy.view_ofs);
			vec = normalize ( vec);
			newmis.velocity = ((vec * newmis.speed) + (v_right * ofs) + (v_up * ofsv));
			newmis.drawflags = MLS_POWERMODE;
			newmis.think = polymorph_anim_white_servant;
			thinktime newmis : 0;
	   }
	   setsize ( newmis, '0.00000 0.00000 0.00000', '0.00000 0.00000 0.00000');

	//OvinoImp mod code end
}

void Use_Polymorph ()
{
	//Inky: white servant fires more than a regular player!
	if(self.classname == "monster_imp_lord"&&self.skin==4)
	{
		FirePoly(-90,0);FirePoly(-50,0);FirePoly(-25,0);FirePoly(25,0);FirePoly(50,0);FirePoly(90,0);
		FirePoly(-150,-50);FirePoly(-90,-50);FirePoly(-25,-50);FirePoly(0,-50);FirePoly(25,-50);FirePoly(90,-50);FirePoly(150,-50);
		FirePoly(-150,75);FirePoly(-90,-50);FirePoly(-25,-50);FirePoly(0,-50);FirePoly(25,-50);FirePoly(90,-50);FirePoly(150,-50);
	}
	//Inky: code below unchanged from the original except the second parameter set to 0 for backward compatibility
	FirePoly(-150,0);
	FirePoly(-75,0);
	FirePoly(0,0);
	FirePoly(75,0);
