...
{
	sound (self.groundentity/*Sound spot*/, CHAN_VOICE, self.noise1, 1, ATTN_NORM);
	self.state = STATE_BOTTOM;
	self.lifetime = time + self.wait;
	if (((self.spawnflags & START_RTRN) && !(self.spawnflags & START_BOTTOM)) ||
		(self.spawnflags & CONTINUE))
	{
		self.groundentity = self; //Sound spot... Inky 20211029 Not sure if self is the right choice here, needs some testing cases
		self.nextthink = self.ltime + self.wait;
		self.think=newplat_go_up;
	}
	if(!self.spawnflags&/*Trigger static*/16) setorigin (self.enemy, self.origin);	
};
...
void() newplat_trigger_use =
{
	...
	self.groundentity = self; //Sound spot
	if (self.state == STATE_BOTTOM)
		newplat_go_up ();
	else
		newplat_go_down ();
...
void() newplat_go_down =
{
	sound (self.groundentity/*Sound spot*/, CHAN_VOICE, self.noise, 1, ATTN_NORM);
	newplat_calc_down();
};
...
void() newplat_center_touch =
{
	...
	self.groundentity = player; /*Sound spot*/
	if (self.state == STATE_BOTTOM)
		newplat_go_up ();
	else
		newplat_go_down ();
...
void() newplat_spawn_inside_trigger =
{
	...
	if(!self.target)
	{
		if (self.spawnflags & CENTER) //Inky 20210112
		{
			...
			setsize (trigger, platcenter - '16 16 0' + '0 0 1' * ((self.maxs_z - self.mins_z)/2), platcenter + '16 16 0' + '0 0 1' * ((self.maxs_z - self.mins_z)/2+32));
		}
		else
...
/*QUAKED func_newplat (0 .5 .8) ? START_BOTTOM STRT_RTRN CONTINUE CENTER
speed	default 150
