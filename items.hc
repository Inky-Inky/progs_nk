...
void weapon_touch (void)
{
	...
	//Inky: 20200305 If block to allow programmatically weapon giving
	if (self.use)
		other = activator; 
...
void ihealth_touch(void)
{
	...
	if (other.health < other.max_health)
	{
		//Inky: 20191222 replaced the following line by the if-else
		//sound (other, CHAN_VOICE, "items/itempkup.wav", 1, ATTN_NORM);
		//other.health += 10;
		if(self.spawnflags&2)
		{
			//Picking a megahealth sounds like a secret! :)
			sound (other, CHAN_VOICE, "misc/secret.wav", 1, ATTN_NORM);
			
			//Megahealth is limited by the maximum possible health
...
void spawn_instant_health(void)
{
	...
	//Inky 20200714 Model not always i_hboost.mdl: megahealth have no model
...
//Inky: 20200409
/*QUAKED item_armor_05 (0 .5 .8) (-8 -8 -45) (8 8 20) FLOATING
-------------------------FIELDS-------------------------

--------------------------------------------------------
*/
void item_armor_05 (void)
{
	switch(cl_playerclass)
	{
		...
		case CLASS_NECROMANCER:
			...
			spawn_item_armor_breastplate();
			break;
		default: //CLASS_ASSASSIN & CLASS_SUCCUBUS
			self.classname = "item_armor_helmet";
			spawn_item_armor_helmet();
			break;
	}
}

/*QUAKED item_armor_10 (0 .5 .8) (-8 -8 -45) (8 8 20) FLOATING
-------------------------FIELDS-------------------------

--------------------------------------------------------
*/
void item_armor_10 (void)
{
	switch(cl_playerclass)
	{
		case CLASS_PALADIN:
...
/*QUAKED item_armor_15 (0 .5 .8) (-8 -8 -45) (8 8 20) FLOATING
-------------------------FIELDS-------------------------

--------------------------------------------------------
*/
void item_armor_15 (void)
{
	switch(cl_playerclass)
	{
		case CLASS_PALADIN:
...
/*QUAKED item_armor_25 (0 .5 .8) (-8 -8 -45) (8 8 20) FLOATING
-------------------------FIELDS-------------------------

--------------------------------------------------------
*/
void item_armor_25 (void)
{
	switch(cl_playerclass)
	{
		case CLASS_PALADIN:
...
void BackpackTouch(void)
{
	...
	if (self.cnt_haste > 0)
	{
		...
		sprint(other,STR_WHITE);
		if (self.cnt_haste > 1)	// Plural
			sprint(other,"s");
		//Inky - end
	}
	if (self.cnt_blast > 0)
...
float it_total;	
	...
	else if (self.monsterclass >= CLASS_LEADER) //Inky: 20200409 Turned == into >=
	{		
		if (random() < .15) // %15 chance he'll drop something	
		{
			chance = random();
...
//		item.velocity_y = random(-20,20);
		...
		else if (item.cnt_haste)
		{
			...
			spawn_artifact (ARTIFACT_WHITE,NO_RESPAWN);
		}
