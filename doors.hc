...
void door_go_down()
{
	entity shadow;	/*20210703 bmFbr's shadows code*/
	entity oldself;	/*20210703 bmFbr's shadows code*/
...
*/
	...
	//Inky 20211027 change to alternate textures
	if(!self.spawnflags & /*Don't change frame*/65536) self.frame = 1 - self.frame;
...
//		return;
	...
	if(other.flags&FL_MONSTER&&world.spawnflags&MISSIONPACK&&!self.owner.spawnflags&262144/*Openable by monsters*/)
		return;
...
void door_killed()
{
	...
	self			= self.owner;
	self.health		= self.max_health;
	//Inky: 20200405 Commented the line below which prevented a multi-use shootable door
...
//		return;
	...
	if(other.flags&FL_MONSTER&&world.spawnflags&MISSIONPACK&&!self.spawnflags&262144/*Openable by monsters*/)
		return;
	...
	//Inky 20211017 Complexified the "if" to allow an inactive door's message to be displayed
	//if(self.owner.attack_finished > time)
	if(self.owner.attack_finished > time && self.inactive==FALSE)
		return;

	//if (self.owner) Inky 2021017 To keep everything functionally equal otherwise
	if (self.owner && self.inactive==FALSE)
	...
	if (self.inactive) //Inky 20211017 Once the message has been displayed, there is nothing more to do for an inactive door
		return;
   ...
   cmaxs = self.maxs;
	...
	do
	{
		...
		// they don't spawn a field
			...
			//Inky: 202000405 Simplified the "if "to prevent the "health" effect from being deactivated by the presence of a thingtype
			//if (!self.thingtype && self.health)
			if (self.health)
				return;
			//Inky: 202000405 Complexified the "if" to allow an initially inactive non shootable door
			//if (self.targetname)
			if (self.targetname) if(!self.inactive || self.health)
				return;
...
void door_sounds(void)
{
	...
	if (self.soundtype == 0)			// No sound by default, but may be overriden by custom sounds
	{
		if(self.noise1) precache_sound2 (self.noise1); else self.noise1 = "misc/null.wav"; //Stop sound
		if(self.noise2) precache_sound2 (self.noise2); else self.noise2 = "misc/null.wav"; //Move sound
		if(self.noise4) precache_sound2 (self.noise4); else self.noise4 = "misc/null.wav"; //Start sound
	}
	...
	else if (self.soundtype == 10)		// Pen door
	{
		precache_sound2 ("doors/wswngstop.wav");
		precache_sound2 ("doors/wswng.wav");

		self.noise1 = "doors/wswngstop.wav";
		self.noise2 = "doors/wswng.wav";
...
void func_door()
{
entity shadow, oldself; /*20210703 bmFbr's shadows code*/
float movedist, num_axes;
	...
	if (!self.lip)
		self.lip = 8;
	if (self.lip>0 && self.lip<1) //Inky 20210921 Trick to have a lip exactly equal to zero (put a dummy 0.1 or w/e value)
		self.lip = 0;
...
// the sizes can be detected properly.
	...
	if(self.switchshadstyle)
	{
		...
		shadow.speed = vlen(self.pos2 - self.pos1) / self.speed * 0.9;
...
void fd_secret_use()
{
	...
	if(self.spawnflags & SECRET_NO_SHOOT) //Inky: 20200407 Was incorrectly if(!self.spawnflags & SECRET_NO_SHOOT)
	{
	...
	if (self.spawnflags & SECRET_1ST_DOWN)
		self.dest1 = self.origin - v_up * (self.t_width - self.frags); //Inky 20210825 Taking lip into account for first move (.frags)
	else
		self.dest1 = self.origin + v_right * ((self.t_width - self.frags) * temp); //Inky 20210825 Taking lip into account for first move (.frags)
		
	self.dest2 = self.dest1 + v_forward * (self.t_length - self.lip); //Inky 20201104 Taking lip into account (.lip)
...
// but spawn in the open position
	...
	//Inky: 20200305 If case was present for func_door but missing for func_door_rotating
	if (self.health)
	{
		self.takedamage = DAMAGE_YES;
		self.th_die =self.th_pain= door_killed;
