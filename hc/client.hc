...
/*		dprint("Resorting to info_player_start\n");
	...
	if (startspot!="")
	{
		dprint("Searching for info_player_start with targetname '");dprint(startspot);dprint("'\n");
		spot = world;
		pcount = 1;
		while(pcount)
		{
		...
		//Inky 20201212 Check for a trigger_changelevel_stealth destination
		if (!spot)
		{
			spot = world;
			pcount = 1;
			while(pcount)
			{
				spot = find (spot, classname, "trigger_changelevel_stealth");
				if (!spot)
					pcount = 0;
				else if (spot.targetname == startspot)
					pcount = 0;
			...
			if(spot)
			{
				//Look for the paired info_player_start entity (whose netname is the trigger's targetname)
				entity info;
				info = world;
				pcount = 1;
				while(pcount)
				{
					info = find (info, netname, spot.targetname);
					if (!info)
						pcount = 0;
					else if (info.classname == "info_player_start2")
						pcount = 0;
				}
				if(info)
				{
					setorigin (info, (spot.absmax+spot.absmin)*0.5 + self.finaldest);
					info.angles=self.angles;
					spot=info;
				}
			}
		}
	...
	dprint("Spawn spot is of class ");dprint(spot.classname);dprint("\n");
...
float pclass;
	...
	dprint("Entering PutClientInServer...\n");
...
//	self.th_stand();
	...
	self.watershift = TRUE; //Inky 20211111 Catch runaway cshifts (changlevel/respawn while in liquid func_train_mp)
};
...
//============================================================================
void() CheckLiquidTrainDeath; //Inky 20211111 Check cshift for dead players in liquid func_train_mp

void() PlayerDeathThink =
{
float		forward;

	//Inky 20211111 Check cshift for dead players in liquid func_train_mp
	CheckLiquidTrainDeath();
...
void() PlayerPreThink =
{
	...
	if (self.zerogtime < time)
		self.gravity = 1;// reset normal gravity
	else
		self.gravity = 0.0001;// zero G (0 OR 1 is normal)
	//Inky 20211111 END  - liquid func_train_mp support
	...
	{
		CheckLiquidTrainDeath(); //Inky 20211111 Check cshift for dead players in liquid func_train_mp
...
//			W_SetCurrentAmmo ();
			...
			//Inky: not undercover anymore
			if(self.flags & FL_NOTARGET)
			{
				centerprint(self, "Not undercover anymore!");
