...
//Inky 20200627 Make the fish catchable in a fishing net!
void fish_catch(void)
{
	entity found;
	if(other.classname=="player")
	{
		if (!client_has_piece(other, "net"))
			return;
		
		if(self.classname=="monster_fish")
			found = find(world, puzzle_id, "fish");
		else if(self.classname=="monster_hydra")
			found = find(world, puzzle_id, "squid");
		
		if(!found)
			return;
		
		if(self.classname=="monster_fish")
			centerprint(other,"You caught a fish!\n");
		else if(self.classname=="monster_hydra")
			centerprint(other,"You caught a squid!\n");
...
//	self.drawflags (+) MLS_POWERMODE;
	...
	self.touch = fish_catch; //Inky 20200627 Make the fish catchable in a fishing net!
	self.thingtype = THINGTYPE_FLESH; //Inky 20200627 for the sake of the next line
	self.th_die = chunk_death; //Inky 20200627 instead of fish_die: cooler! ;-)
	self.think = fish_hover;
	thinktime self : HX_FRAME_TIME;
}
