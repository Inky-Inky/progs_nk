...
float too_close;
	...
	if((self.skin>2&&self.enemy==self.controller&&vlen(self.enemy.origin-self.origin)<128)||(self.goalentity==world||self.enemy==world))
	{
...
void imp_missile ()
{
	//Inky: "if" part is the white servant behavior, "else" part is the orginal behavior
	if(self.classname == "monster_imp_lord"&&self.skin==4)
	{
         ...
         Use_Polymorph( );
		 self.enemy = world; //Forget the current target, now it has been polymorphed
		 FindMonsterTarget(); //Now look for a new target
	}
	else
	{
		//Inky: below the original method's code
...
float dist,damg;
	...
	string hitsound;
		...
		{
			trace_ent.thingtype==THINGTYPE_ICE;
			MeatChunks (trace_endpos,v_right*random(-100,-300)+'0 0 200', 3,trace_ent);
			hitsound="weapons/slash.wav";
		}
		else
...
float too_close;
	...
	if(!self.enemy||(self.enemy==self.controller&&self.skin>2))
		if(imp_find_target())
			return;
