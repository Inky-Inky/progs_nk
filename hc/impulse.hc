...
{
		entity search = nextent(world);
		float total = 0;

		while(search != world)
		{
			if (search.flags & FL_MONSTER && search.health > 0)
			{
				total += 1;
				T_Damage (search, world, world, search.health);
			}
			search = nextent(search);
		}
		bprint("Killed ");bprint(ftos(total));bprint(" monsters\n");
}
	...
	else if (self.impulse==35&&skill<3)
	{
		...
		while(search != world)
		{
			if (search.flags & FL_MONSTER)
			{
				total += 1;
				//remove(search);
				T_Damage (search, world, world, 2*search.health);
			}
			search = nextent(search);
		}
		dprintf("Killed %s monsters\n",total);
	}
...
/*	else if (self.impulse == 99)
	...
	else if (self.impulse == 254)
	{
		...
		*/
		...
		bprint("Player velocity: ");
		bprint(vtos(search.velocity));
		bprint("\n");
	}
	else if (self.impulse == 255)
		PrintFrags();
	//Inky: 20200608 RPG additions (self.worldtype stores the user's choice)
	else if (self.impulse >= 1 && self.impulse <= 4 && self.plaqueflg==1)
	{
		self.worldtype = self.impulse;
		self.impulse=0;
	}
	//Inky: RPG additions - END
...
//		else
	...
	else if (self.impulse == 15) //Inky 20210922
	{
		vector org = self.origin + '0 0 16';
		traceline (org, org + v_forward*5000, FALSE, self);
		
		jprint("Hit ");jprint(trace_ent.classname); if (trace_ent.targetname) { jprint(" ");jprint(trace_ent.targetname); }
		if (trace_ent.flags & FL_MONSTER || trace_ent.health > 0)
		{
			jprint(" and killing it\n");
			T_Damage (trace_ent, self, self, trace_ent.health);
		}
		else if (trace_ent.classname == "func_wall")
		{
			remove(trace_ent);
		}
		else if (trace_ent.touch)
		{
			jprint(" and touching it\n");
		...
		else if (trace_ent.use)
		{
			...
			jprint(" and having no idea what to do with it\n");
	}
	else if (self.impulse == 16) //Inky 20210922
	{
		KillAllMonsters();
	}
	else if (self.impulse == 17) //Inky 20210922
	{
		vector org2 = self.origin + '0 0 16';
		traceline (org2, org2 + v_forward*5000, FALSE, self);
		
		jprint("Hit ");jprint(trace_ent.classname);
		if(trace_ent!=world)
		{
			if (trace_ent.targetname) { jprint(" ");jprint(trace_ent.targetname); }
			remove(trace_ent);
			jprint(" and removing it\n");
		}
	}
