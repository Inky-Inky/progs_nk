...
void CreateModelChunks (vector space,float scalemod, float numChunks)
{
	...
	else if (self.thingtype==THINGTYPE_FLESH || self.thingtype==THINGTYPE_GREENFLESH)
	{
		if(self.model=="models/spider.mdl" || self.thingtype==THINGTYPE_GREENFLESH)
		{
			if (final < 0.33)
				setmodel (chunk, "models/sflesh1.mdl");
			else if (final < 0.66)
	...
	else if (self.thingtype==THINGTYPE_BROWNSTONE)
	{
		...
		//Inky: 20200408 Now possible to choose the skin
		//chunk.skin = 1;
		if(self.skin)
	...
	else if (self.thingtype==THINGTYPE_CLOTH)
	{
		...
		//Inky 20201122 For rugs and bear skin in the yurt
		chunk.skin = self.skin;
	}
	...
	else if (self.thingtype==THINGTYPE_ICE) //Inky: 20200409 Introduced skin 3
	{
...
entity DropBackpack(void);  // in items.hc
...
//	BloodSplat();
	...
	else if (self.thingtype==THINGTYPE_FLESH || self.thingtype==THINGTYPE_GREENFLESH)//||(self.thingtype==THINGTYPE_ACID&&self.flags2&FL_ALIVE))
	{
	...
	else if ((self.thingtype==THINGTYPE_LEAVES)  || (self.thingtype==THINGTYPE_WOOD_LEAF) || (self.thingtype==THINGTYPE_ICE && self.skin==3)) //Inky: 20200409 Special sound for snow
	...
	//Inky 20201218 Recoil
	if(self.origin=='0 0 0') self.origin = (self.absmax+self.absmin)*0.5;
	if(self.ring_flight && (damage_attacker.classname=="player"||damage_attacker.flags&FL_MONSTER) && vlen(damage_attacker.origin-self.origin) < self.ring_flight)
	{
		jprint("Recoil!\n");
		self.ring_flight-=vlen(damage_attacker.origin-self.origin);
		damage_attacker.velocity+=normalize(v_forward)*-self.ring_flight+normalize(v_up)*-self.ring_flight*0.3;
	}
	...
	{
		entity delayedlight=spawn();
		delayedlight.switchshadstyle = self.switchshadstyle;
		delayedlight.think=let_light_pass;
		thinktime delayedlight : 0.1;
	}
