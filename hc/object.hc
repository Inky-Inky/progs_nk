...
{
	//if(self.skin==4)
	if(self.skin==-1) //Inky 20201122 Move giant web to slot -1 to free values >=4 for custom skins
	{
...
//Inky: a hanging objet (ie whose initial Movetype is 0-none) becomes 13-pushable and it falls down
void hanging_object_falls_down(entity attacker, float damage)
{
	self.movetype = MOVETYPE_PUSHPULL;
}

//Inky: a hanging objet (ie whose initial Movetype is 0-none) becomes 13-pushable and it falls down
void hanging_object_falls_down_use()
{
	self.movetype = MOVETYPE_PUSHPULL;
}
//End
...
void obj_beefslab (void)
{
	...
	//Inky: an obj_beefslab set to "pushable" (spawnflags & 1) has an additional behavior: as soon as hit it falls down
	if(self.spawnflags&1)
		self.th_pain = hanging_object_falls_down;
	//End
}
...
float solidity; //Inky
	...
	self.drawflags(+)SCALE_ORIGIN_BOTTOM;
	//Inky: self.use now depending on spawnflags
	//self.use=skeleton_throne_use;
	if (self.spawnflags&1)
	{
		solidity = SOLID_NOT;		// no interaction with other objects
		self.use = chunk_death;
	}
	else
	{
		solidity = SOLID_BBOX;		// touch on edge, block
		self.use=skeleton_throne_use;
	}
	...
	makevectors(self.angles);
	box2=spawn();
	box2.solid=solidity; //Inky: previously SOLID_BBOX
	setsize(box2,'-26 -26 0','26 26 36');
	setorigin(box2,self.origin-v_forward*7);
	box3=spawn();
	box3.solid=solidity; //Inky: previously SOLID_BBOX
...
//Inky: Skeletal King's throne as an independent standalone oject
void obj_throne (void)
{
	precache_model4("models/throne.mdl");
	CreateEntityNew(self,ENT_SKELTHRN,"models/throne.mdl",chunk_death);
}
