...
void ScorpionInit(float type)
{
	...
	//self.touch = SUB_Null;
	//self.use = SUB_Null;
	if(self.monsterclass==CLASS_BOSS)
	{
		self.scale=20;
		setsize(self, '-48 -48 -48', '48 48 64');
...
void ScorpionMelee(float damage)
{
	...
	source = self.origin;
	traceline (source, source + v_forward*60*self.scale, FALSE, self); //Inky 20200722 Added *self.scale for the giant scorpions

	if (trace_ent != self.enemy) return;

	damage += random(0.1, 1);
	
	if(self.monsterclass==CLASS_BOSS) //Inky 20200708 Let's make super scorpions a bit more dangerous!
		damage = damage * 3;
