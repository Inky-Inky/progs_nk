...
float move_cnt;
	...
	entity oself;
		...
		thinktime oself : 0;
		self=newmis;
		spawn_artifact(ARTIFACT_WHITE,FALSE);
	}
}
...
entity missile;
    ...
    missile.movetype=MOVETYPE_BOUNCE;
    missile.solid=SOLID_BBOX;
	missile.touch=BecomeWhiteServant;
	missile.effects=EF_DIMLIGHT;
	missile.drawflags=MLS_POWERMODE;
	...
	setmodel (missile, "models/a_servant.mdl");
	setsize(missile,'0 0 0','0 0 0');
	setorigin(missile,self.origin+self.proj_ofs+v_forward*16);
	missile.think=BecomeWhiteServant;
	thinktime missile : 1;
	self.cnt_haste-=1;
}
