...
//Inky 20201213 Refined god mode handling
...
void EnterGodMode()
{
	...
	dprint(self.targetname);dprint(" entering god mode at t+");dprint(ftos(time));dprint("\n");
	self.drawflags (+) DRF_TRANSLUCENT; //Inky 20201213 Adding a hint that the yakman entered god mode
	self.drawflags (+) MLS_POWERMODE;
	self.invincible_time=time;
	self.healthtime = 0;
}

void ExitGodModeIfNeeded(float forceExit)
{
	if(forceExit || (self.invincible_time > 0 && time > self.invincible_time + 2.5))
	{
		self.divine_time = time;
		self.drawflags (-) DRF_TRANSLUCENT; //Inky 20201213 Adding a hint that the yakman got back to normal mode
		self.drawflags (-) MLS_POWERMODE;
		dprint("Yakman ");dprint(self.targetname);dprint(" leaving god mode at t+");dprint(ftos(self.divine_time));dprint("\n");
		self.takedamage=DAMAGE_YES;
		self.invincible_time = 0;
		self.healthtime = time;
	}
}
...
{//FIXME, maybe deflect stuff?
	...
	//if(self.cnt>=11) Inky 20201213 The original condition meant a 0.5 seconds invulnerability, making this super power practically useless. Too short! Lets make things a bit more interesting...
	if(time > self.invincible_time + 2.5)
	{
		self.cnt=0;
		ExitGodModeIfNeeded(TRUE);
...
entity enemy_proj;
	...
	else if(self.frame>=$walk5 &&self.frame<=$walk10 && time > self.healthtime + 5  && self.scale!=2) //Inky 20201213 Yakman won't enter god mode again if last time it exited was less than 5 seconds ago. Giant yaks never enter god mode anyway.
	{
		EnterGodMode();
		self.frame=$bwalk7;
	...
	else if(self.frame>=$walk17 &&self.frame<=$walk22 && time > self.healthtime + 5  && self.scale!=2) //Inky 20201213 Yakman won't enter god mode again if last time it exited was less than 5 seconds ago. Giant yaks never enter god mode anyway.
	{
		EnterGodMode();
		self.frame=$bwalk19;
		self.think=yakman_block;
...
//Head rear anim
	...
	T_Damage(loser,self,self,5*(/*self.skin*/self.frags+2)*(self.aflag+1)*(coop + 1)); //Inky 20201107
...
void yakman_pain (entity attacker, float damage)
{
	...
	sound(self,CHAN_VOICE,"yakman/pain.wav",1,ATTN_NORM);
		self.pain_finished=time+1+/*self.skin*/self.frags/2; //Inky 20201107
...
//	ai_face();
	...
	{bprint("Ano\n");//debug
bprint(vtos(self.velocity));//debug
bprint("On s'arrete la.\n");//debug
remove(self);//debug
...
float enemy_dist;
	...
	if(enemy_dist>256&&random()<0.3 - /*self.skin*/self.frags/10) //Inky 20201107
	{//Shoot
		self.touch=SUB_Null;
		self.weaponframe_cnt=0;
		thinktime self : 0;
	...
	{
		if(trace_ent.takedamage && 64 >= vlen ((self.origin + self.view_ofs) - (trace_ent.origin + trace_ent.view_ofs)))
			yakman_slam(trace_ent);
		else if(CheckJump(TRUE))
		{
			yakman_jump (); //Inky 20201206 Seems less pointless than the previous line below
			//return;//??
		}
		else //if(trace_ent.solid==SOLID_BSP)
		{
			local vector try_left, try_right, trace_ent_closest_bound;
			float space_left, space_right, step;
			...
			if(time > self.lifetime)
			{
				...
				do
				{
					try_left = try_right = self.angles;
					try_left_y  -=step;
					try_right_y +=step;
					
					//Try to the left
					makevectors(try_left);
					traceline(self.origin, v_forward*256, TRUE, self);
					trace_ent_closest_bound = self.origin + (vlen(self.origin - trace_ent.origin)-max(fabs(trace_ent.absmax_x-trace_ent.absmin_x),fabs(trace_ent.absmax_y-trace_ent.absmin_y))*0.5)*v_forward;
					space_left = vlen(self.origin - trace_ent_closest_bound);
					
					//Try to the right
					makevectors(try_right);
					traceline(self.origin, v_forward*256, TRUE, self);
					trace_ent_closest_bound = self.origin + (vlen(self.origin - trace_ent.origin)-max(fabs(trace_ent.absmax_x-trace_ent.absmin_x),fabs(trace_ent.absmax_y-trace_ent.absmin_y))*0.5)*v_forward;
					space_right = vlen(self.origin - trace_ent_closest_bound);
					
					if(space_right >= max(space_left,10+32*self.scale))
					{
						step=0;
						//CreateRedFlash(self.origin + '0 0 40');
						setorigin(self, self.origin+normalize(v_forward)*min(space_right/2,64));
						//self.lifetime=time+0.4-skill/10; //Slow down the tempo at which the Yakman chains its teleportations
					}
					else if(space_left >= max(space_right,10+32*self.scale))
					{
						step=0;
						makevectors(try_left);
						//CreateRedFlash(self.origin + '0 0 40');
						setorigin(self, self.origin+normalize(v_forward)*min(space_left/2,64));
						//self.lifetime=time+0.4-skill/10; //Slow down the tempo at which the Yakman chains its teleportations
					}
					else if (step == 360)
					{
						step = 0;
						self.lifetime=time+3; //Yakman won't chain-teleport more often than every 3 seconds
					}
					else
						step += 30;
				}
			...
			//movetogoal(7); Inky 20201206 Try the 2 lines above instead
...
vector dir;
	...
	vector punchdir,throwdir;
		...
		*/
		...
		T_Damage(trace_ent,self,self,2*(/*self.skin*/self.frags+1)*(coop + 1)); //Inky 20201107
	}
...
{
	ExitGodModeIfNeeded(FALSE);
	if((fabs(self.enemy.origin_z-self.origin_z)<48&&vlen(self.origin-self.enemy.origin)<512&&random()<0.7+/*self.skin*/self.frags/10)||self.enemy.frozen>0||self.scale==2) //Inky 20200808 20201107
	{
		if(vlen(self.origin-self.enemy.origin)<100||self.scale==2) //Inky 20200830
...
skin:	0 - white: more often uses ice, and less often charges
		...
		3 - shambler-like, even more fast and powerful! (added by Inky)
		4 - shepherd, twice the normal size, melee only and super tough (added by Inky)
*/
...
*/
	...
	else if(self.skin == -1) //Inky 20200708 Skin 0 was not settable because considered as "no skin selected" and the previous "if" applied
	{
		self.skin=0;
	}
	//Inky: skin == 3 & 4 now legal
	else if(self.skin>4)
	//else if(self.skin>2)
	{
...
//	self.scale=.8;
	self.drawflags(+)SCALE_ORIGIN_BOTTOM; //Inky 20200808 Was commented out. Can't figure out why. Needed anyway for giant one (scale 2)
...
//	self.hull=HULL_SCORPION;
	...
	//Inky 20201107 damage caused was mathematically related to skin index. With the new skins added, that would be really lots of damage so let's cut it off.
	if(self.skin>2)
		self.frags=2;
	else
		self.frags=self.skin;
