...
float		r,melee;	
	...
	//Inky: 20200330 the distance doesn't matter for good snipers!
	if (self.intelligence&&self.classname!="monster_golem_iron")
		return RANGE_MELEE;
...
//	newmis.spawnflags=NO_DROP;
	...
	setorigin(newmis,org);
	if(!(self.controller.spawnflags&QUIET || self.spawnflags&QUIET))
	{
		if(self.spawnflags&65536/*Big teleport effect*/)
			GenerateTeleportEffect(org,0);
		else
			spawn_tfog(org);
	}
	
	//Inky 20211030 Angry at player right away!
	if(self.spawnflags&262144/*Angry at player*/)
	{
...
//Inky 20201206 Use when a monster is blocked. It will then look for the closest possible yaw to escape the dead-end.
float FindFreeYaw (void)
{
	local vector try_left, try_right;
	float space_left, space_right;
	
	try_left = try_right = self.angles;
	try_left_z = try_right_z = 0;
	try_left  += '0 -45 0';
	try_right += '0 45 0';
	...
	space_left = vlen(self.origin - trace_ent.origin);
	
	//Try to the right
	makevectors(try_right);
	traceline(self.origin, v_forward*256, TRUE, self);
	space_right = vlen(self.origin - trace_ent.origin);
	
	if(space_right - space_left>128)
	{
		self.ideal_yaw = self.angles_y + 45;
		return -1;
	}
	else if(space_left - space_right>128)
	{
		self.ideal_yaw = self.angles_y - 45;
		return 1;
	}
	...
	{
		space_left=rint(random(-1,1));
		self.ideal_yaw = self.angles_y + space_left * 90;
		return 1 - space_left;
	}
